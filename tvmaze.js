...
async function getShowsByTerm(term) {
  ...
  for(let i = 0; i< showArr.length; i++) {    
      let showData = {};
      showData.id = showArr[i].show.id;
      showData.name = showArr[i].show.name;
      showData.summary =showArr[i].show.summary;
      showData.image = showArr[i].show.image.medium;
      resultArr.push(showData);
  }
  console.log(resultArr[0].image);
  return resultArr;
}
...
function populateShows(shows) {
  ...
  for (let show of shows) {
    if (show.image === null || show.image === undefined) {
